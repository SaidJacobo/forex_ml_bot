...
class MACD():
    ...
    def calculate_operation_sides(self, prices_with_indicators):
        ...
        long_signals = (
            ...
            & (df['Close'] > df['sma_200'])
            & (df['diff_pips_sma_200_26'] > 15)
            & (df['diff_pips_sma_200_26'] > df['diff_pips_sma_200_26'].shift(1))
            & (df['diff_pips_sma_200_26'].shift(1) > df['diff_pips_sma_200_26'].shift(2))
            & (df['diff_pips_sma_200_26'].shift(2) > df['diff_pips_sma_200_26'].shift(3))
            & (df['diff_pips_sma_200_26'].shift(3) > df['diff_pips_sma_200_26'].shift(4))
        )
    ...
    def _calculate_units_size(self, balance, total_orders, price):
        ...
        units = 1000 * loss_orders * self.multiplier
        units = 1000 if units == 0 else units
