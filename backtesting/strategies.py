...
class LimitFvg(Strategy):
    ...
    def next(self):
        ...
        if self.position:
            ...
            if bullish_fvg:
                self.buy(
                    size=self.risk / 100, limit=actual_close
                )
...
class Bbands(Strategy):
    ...
    def next(self):
        ...
        if self.position:
            ...
            if self.position.is_long:
                if actual_close >= self.upper_band or time_in_position >= 5:
                    self.position.close()

            if self.position.is_short:
                if actual_close <= self.lower_band or time_in_position >= 5:
                    self.position.close()

        else:

            if actual_close <= self.lower_band and actual_close > self.daily_sma_200:
                self.buy(size=self.risk / 100)
                
            if actual_close >= self.upper_band and actual_close > self.daily_sma_200:
                self.sell(size=self.risk / 100)